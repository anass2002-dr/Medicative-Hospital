DROP DATABASE IF EXISTS beautymedic;
CREATE DATABASE beautymedic;
USE beautymedic;
...
CREATE TABLE IF NOT EXISTS `collection_photos` (
  ...
  KEY `FK_COLLECTION_PHOTO_BLOG` (`BLOG_ID`)
...
CREATE TABLE IF NOT EXISTS `comment` (
  ...
  KEY `FK_COMMENTING` (`USER_ID`),
  KEY `FK_CONTAINE` (`BLOG_ID`)
...
CREATE TABLE IF NOT EXISTS `product_collection_photos` (
  `PD_COLLECTION_ID` int(11) NOT NULL AUTO_INCREMENT,
  `PRODUCT_ID` int(11) DEFAULT NULL,
  `PHOTO_PATH` text DEFAULT NULL,
  PRIMARY KEY (`PD_COLLECTION_ID`),
...
ALTER TABLE `blog`
  ADD CONSTRAINT `FK_BLOG_CATEGORY` FOREIGN KEY (`CATEGORY_ID`) REFERENCES `category` (`CATEGORY_ID`)ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE `collection_photos`
  ADD CONSTRAINT `FK_COLLECTION_PHOTO_BLOG` FOREIGN KEY (`BLOG_ID`) REFERENCES `blog` (`BLOG_ID`)ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE `comment`
  ADD CONSTRAINT `FK_COMMENTING` FOREIGN KEY (`USER_ID`) REFERENCES `user` (`USER_ID`)ON UPDATE CASCADE ON DELETE CASCADE;
 ALTER TABLE `comment`
 ADD CONSTRAINT `FK_CONTAINE` FOREIGN KEY (`BLOG_ID`) REFERENCES `blog` (`BLOG_ID`)ON UPDATE CASCADE ON DELETE CASCADE;
