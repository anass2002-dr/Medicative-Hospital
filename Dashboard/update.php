...
if (!empty($title) and !empty($category) and !empty($blog)) {
    ...
    if(!empty($video)){
        if(!empty($photo)){
            $query="UPDATE blog SET TITLE='$title',CATEGORY_ID='$category',PHOTO='$photo',VIDEO='$video',PRODUCT_LINK='$product_link',CONTENT='$blog',CREATED_DATE='$date' where BLOG_ID=$id";
        }
        else{
            $query="UPDATE blog SET TITLE='$title',CATEGORY_ID='$category',VIDEO='$video',PRODUCT_LINK='$product_link',CONTENT='$blog',CREATED_DATE='$date' where BLOG_ID=$id";
        }
    }
    else{
        if(!empty($photo)){
            $query="UPDATE blog SET TITLE='$title',CATEGORY_ID='$category',PHOTO='$photo',PRODUCT_LINK='$product_link',CONTENT='$blog',CREATED_DATE='$date' where BLOG_ID=$id";
        }
        else{
            $query="UPDATE blog SET TITLE='$title',CATEGORY_ID='$category',PRODUCT_LINK='$product_link',CONTENT='$blog',CREATED_DATE='$date' where BLOG_ID=$id";
        }
    }
    // $conn->query($query);
